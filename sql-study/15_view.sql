...
-- 뷰의 컬럼명은 함수 호출문, 연산식 등 같은 기상 표현식이면 안됨
SELECT 
    employee_id, 
...
-- 뷰의 수정 (CERATE OR REPLACE VIEW 구문)
...
-- 두 번째 컬럼인 'full_name'은 가상열(virtual column)이기 때문에 INSERT가 안됨
INSERT INTO view_emp_dept_job
VALUES(300, 'test', 'test', 'test', 10000); --  에러

-- JOIN 된 뷰(복합 뷰)의 경우에는 한번에 삽입할 수 없다.
INSERT INTO view_emp_dept_job
    (employee_id, department_name, job_title, salary)
VALUES(300, 'test', 'test', 10000); -- 에러

-- 원본 테이블 컬럼 중 NOT NULL 컬럼이 존재하고, 해당 컬럼을 view로 지목할 수 없다면 INSERT가 불가함 
