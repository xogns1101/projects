...
-- 이름이 'Nancy'인 사원 보다 급여가 높은 사원을 구하기
...
-- 단일 행 연산자 : 주로 비교 연산자(=, >, <, <=, >=, <>)를 사용하는 경우 하나의 행만 반환해야 함
-- 이런 경우에는 다중 행 연산자를 사용해야 함
SELECT * FROM employees
WHERE job_id = (SELECT job_id FROM employees
                WHERE employees.last_name = 'King'); -- 에러
...
-- EXISTS : 서브쿼리가 하나 이상의 행을 반환 하면 참으로 간주
-- job_history에 존재하는 직원이 employees에도 존재한다면 조회대상에 포함
-- 서브쿼리 내에서 jh에 있는 id와 e에 있는 id가 일치할 때마다 1이라는 값을 조회
-- EXISTS 연산자가 1이 조회가 될 때 데이터가 존재 한다는 것을 판단하여 employees에서 해당 사원의 모든 정보를 조회
