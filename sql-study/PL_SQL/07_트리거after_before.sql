...
-- BOFORE 트리거
-- ROLLBACK은 BEGIN 안에서는 사용이 안되고 밖에서 사용해야함
CREATE OR REPLACE TRIGGER trg_user_insert
    BEFORE INSERT
    ON tbl_user
    FOR EACH ROW 
BEGIN
    -- INSERT 예정인 name의 값에서 첫글자만 추출 후 뒤에 *을 세게 붙임 
...
BEGIN
    ...
    -- 재고 수량이 넉넉하다면 주문 수량 만큼 재고 수령을 조정
    UPDATE product SET product.total = product.total - v_total
    WHERE product.product_no = v_product_no;
    ...
    EXCEPTION
        WHEN quantity_shortage_exception THEN
            ...
            -- 두번째 매개값: 에러 메세지
            RAISE_APPLICATION_ERROR(-20001, '주문 하신 수량보다 갯수가 적어 주문이 불가합니다.');
        WHEN zero_total_exception THEN
            RAISE_APPLICATION_ERROR(-20002, '주문 하신 상품의 재고가 소진되어 주문이 불가합니다.');
END;
...
-- 트리거 내에서 예외가 발생하면 수행중인 INSERT 작업은 중단되며 ROLLBACK이 진행 됨
