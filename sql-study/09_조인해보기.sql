...
*/
-- employees 테이블 부서 id 와 일치하는 departments 테이블의 부서 
-- id를 찾아서 SELECT 이하에 있는 컬럼들을 출력하는 쿼리문
SELECT
    e.first_name,
    d.department_name
FROM employees e, departments d
WHERE e.department_id = d.department_id; -- 오라클의 조인 문법
...
AND loc.state_province = 'California'; -- 일반조건은 조인 조건 이후에 작성 해준다

-- 외부 조인
/*
상호 테이블간에 일치되는 값으로 연결되는 내부 조인과는 다르게
어느 한 테이블에 공통 값이 없더러도 해당 row들이 조회 결과에
모두 포함되는 조인을 말함
*/
SELECT
    e.first_name,
...
-- 외부 조인 진행 시 모든 조건에 (+)를 붙혀야 하며
-- 일반 조건에도 (+)를 붙히지 않으면 데이터가 누락되는 현상이 발생
SELECT
