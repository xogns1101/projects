...
public class WeatherService {
    ...
    public void getShortTermForecast(String area1, String area2) {
        ...
        try {
            ...
            urlBuilder.append("?" + URLEncoder.encode("serviceKey", "UTF-8") + " = wxNd53XeLl8o0O7cDWKZgw08DLradmwhjTRGZN%2B2H%2BjbKkatBGYQDlaZWSFOEpp5AP4TM6YtoVbF8Rw72wJPkA%3D%3D"); /*Service Key*/
           ...
           // System.out.println(sb.toString());
            ...
            // "response" 라는 이름의 키에 해당하는 JSON 데이터를 가져 오기
            JSONObject response = (JSONObject) jsonObject.get("response");

            // response 안에서 body 키에 해당하는 JSON 데이터를 가져오기
            ...
            // item 데이터는 여러 값이기 때문에 배열의 문법을 제공하는 객체로 받는다
            JSONArray itemArray = (JSONArray) items.get("item");
            
            // 반복문을 이용해서 객체를 하나씩 취득한 후에 원하는 로직을 작성하기
            for (Object obj : itemArray) {

                // Object 를 JSON 객체로 변환
                JSONObject item = (JSONObject) obj;

                // "category" 키에 해당하는 단일 값을 가져오기
                String category = (String) item.get("category");

                // "fcstValue" 키에 해당하는 단일 값을 가져오기
