...
public class Score {
    ...
    // 객체가 생성될 때 dto를 전달 받아서 자신의 메서드로 모든 필드를 초기화 하는 생성자를 선언
    public Score(ScoreRequestDTO dto){
        convertInputData(dto); // 입력 받은 값으로 필드 초기화
        calculateTotalAverage();// 총점 평균 계산
    ...
    // 전달되는 dto에서 팔요한 데이터를 Score의 필드에 할당하는 메서드
