...
<div class="container wrap">
          ...
          <form action="/members/sign-up" name="signup" id="signUpForm" method="post"
                ...
                <div class="profile">
                  <div class="thumbnail-box">
                    <img src="/assets/img/image.png" alt="프로필 썸네일">
                  </div>
  ...
  <script>
    ...
    $idInput.onkeyup = e => {
      ...
      if(idValue.trim() === ''){
        ...
        // 패턴과 일치하는 입력값이면 true, 하나라도 어긋난다면 false 리턴 
        $idInput.style.borderColor = 'red';
        ...
        // 비동기 요청 으로 아이디 중복 확인 진행
        fetch('/members/check/account/' + idValue)
            .then(res => res.json())
            .then(flag => {
              if(flag){
                $idInput.style.borderColor = 'red';
                document.getElementById('idChk').innerHTML = '<b style="color: red;">[아이디가 중복 되었습니다.!]</b>';
                checkResultList[0] = false;

              }else{
                $idInput.style.borderColor = 'green';
                document.getElementById('idChk').innerHTML = '<b style="color: green;">[사용 가능한 아이디 입니다!]</b>';
    ...
    const passwordPattern = /([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~])|([!,@,#,$,%,^,&,*,?,_,~].*[a-zA-Z0-9])/;
      ...
      $pwInput.onkeyup = e => {
        ...
        if (pwValue.trim() === '') {
          ...
          document.getElementById('pwChk').innerHTML
                  = '<b style="color: green;">[사용가능한 비밀번호입니다.]</b>';
      ...
      $nameInput.onkeyup = e => {
        ...
        if (nameValue.trim() === '') {
          ...
          document.getElementById('nameChk').innerHTML
                  = '<b style="color: green;">[사용가능한 이름입니다.]</b>';
  ...
  // 이미지 영역을 클릭하면 input-file을 클릭한 것과 동일한 효과를 내기
  $profile.onclick = e => {
    $fileInput.click();
  } 
   
  // 프로필 사진 첨부 시 썸내일 보여주기
  $fileInput.addEventListener('change', e => {
    ...
    // 첨부파일의 바이트 데이터를 읽어드리는 객체 생성
    const reader = new FileReader();

    // 파일의 바이트 데이터를 읽어서 img 태그의 src 속성에 넣으려면
    // url 형태로 전달해야 하는데 , 이걸 처리하는 함수를 사용
